...
export async function GET(request: Request) {
  try {
    ...
    const url = `https://www.jisilu.cn/data/qdii/qdii_list/C?___jsl=LST___t=${Date.now()}&rp=${rp}&page=${page}`;
    ...
    console.error('Error fetching QDII commodity data:', error);
    return NextResponse.json(
        { error: 'Failed to fetch QDII commodity data' },
        { status: 500 }
    );
  }
}
